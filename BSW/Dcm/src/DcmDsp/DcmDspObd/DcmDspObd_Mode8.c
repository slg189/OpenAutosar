...
Std_ReturnType Dcm_DcmObdMode08 (Dcm_SrvOpStatusType OpStatus,Dcm_MsgContextType * pMsgContext,Dcm_NegativeResponseCodeType * dataNegRespCode_u8)
{
    ...
    uint32 dataTidMaskVal_u32;      /* For supported TIDs,each TID is bit coded value of 4bytes */
    uint32 dataCalTidBitMask_u32;   /* Calculated Bit mask based on TID value                   */
    Dcm_MsgLenType nrReqDataLen_u32;      /* Requested data length check                              */
    uint16_least idxTidRes_qu16;     /* Index to response buffer for valid TIDs                  */
    uint8_least nrTid_qu8;      /* Number of TIDs needs to be searched in TID configuration */
    uint8_least idxTidStart_qu8;      /* Start index in TID configuration                         */
    uint8_least nrNumTids_qu8;          /* Number of TIDs in configuration structure                */
    uint8_least nrTidChk_qu8;      /* Number of requested Tids that is to be searched in config structure */
    uint8 adrTmpBuf_au8[DCM_OBDMODE08_REQ_LEN_MAX]; /* Temp buffer maximum of 6 TIDs         */
    uint8 idxTID_u8;         /* Tid index in bitmask configuration list                  */
    uint8 nrMultiple_u8;       /* Number of special TIDS requested are multiples of 0x20   */
    Std_ReturnType stRetTID_u8;            /* Return type of TID which triggers turn ON/OFF of test    */
    boolean stTIDFound_u8;       /* Set to 1 if requested TID is found in configuration      */
    Std_ReturnType dataRetval_u8;            /* Return type of TID which triggers turn ON/OFF of test    */
    ...
    *dataNegRespCode_u8   = 0x0u;
    ...
    /* TRACE [SWS_Dcm_00417],[SWS_Dcm_00418] */
						   ...
						   /* Copy TID Data Record from Request into Buffer */
							...
							if(Dcm_Dsp_Mode8TidArray_acs[nrTid_qu8].UsePort_b != FALSE)
							{
							    ...
							    if(nrReqDataLen_u32 != 1u)
							    {
                                    ...
                                    stRetTID_u8 = (*(Dcm_Dsp_Mode8TidArray_acs[nrTid_qu8].adrRequestControl_cpv.RequestControl1_pf))
