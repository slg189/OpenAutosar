...
#include "Dcm_MemMap.h"
static Dcm_DspRoeState_ten Dcm_stDspRoeState_en;                  /* State for ROE state machine */
static Dcm_DspRoeEventState_ten s_DcmOldRoeDtcState;
#define DCM_STOP_SEC_VAR_CLEARED_UNSPECIFIED /*Adding this for memory mapping*/
#include "Dcm_MemMap.h"
#define DCM_START_SEC_VAR_CLEARED_8 /*Adding this for memory mapping*/
#include "Dcm_MemMap.h"
static uint8 Dcm_stRoeEventWindowTime_u8; /* Variable to store the Roe Event window Time */
static uint8 Dcm_stEventType_u8; /* Variable to store the Roe Event Type */
static Dcm_OpStatusType Dcm_stRoeOpstatus_u8; /* Variable to store the Roe Opstatus*/
#define DCM_STOP_SEC_VAR_CLEARED_8 /*Adding this for memory mapping*/
#include "Dcm_MemMap.h"
#define DCM_START_SEC_VAR_CLEARED_BOOLEAN /*Adding this for memory mapping*/
#include "Dcm_MemMap.h"
static boolean Dcm_stDspRoeStorageState_b; /* Variable to store the storage bit information */
static boolean Dcm_stDspRoeDidOpStatusIsPending_b; /* Variable to store the storage bit information */
static boolean Dcm_stDspRoeDtcOpStatusIsPending_b; /* Variable to store the storage bit information */
        ...
        Dcm_NegativeResponseCodeType * dataNegRespCode_u8)
{
    ...
    if(Dcm_stDspRoeDidOpStatusIsPending_b == FALSE)
    {
        ...
        (*(DcmDspRoeDidEvents[dataDoeDidIdx_u8].ROEDID_cpv.ROEDID_fp))(DcmDspRoeDidStateVariables[dataDoeDidIdx_u8].RoeEventStatus);
#endif
        ...
        Dcm_NegativeResponseCodeType * dataNegRespCode_u8)
{
    ...
    boolean dataStoreEventInfo_b; /*Local variable to chk if event of Did in the loop was either
                                                    started/stopped/cleared*/
    Dcm_SesCtrlType dataCurrentSession_u8;
    uint8 dataidxLoop_u8;
    Std_ReturnType dataReturnVal_u8; /* Local variable to store return from DcmAppl API */
    ...
    *dataNegRespCode_u8 =0x0u;
    ...
    {
        dataStoreEventInfo_b=FALSE; /* Resetting Data Store Event info */
...
{
    Std_ReturnType dataReturnVal_u8; /* Local variable to store return from DcmAppl API */
    uint16 dataRoeDid_u16; /* Local variable to store Response Length */
    uint16 StoreVar_u16;
    uint8 dataidxLoop_u8; /* Index for loop counter */
    Dcm_stDspRoeDidOpStatusIsPending_b = FALSE;
...
static Std_ReturnType Dcm_ControlRoeDtcEventType(
        Dcm_NegativeResponseCodeType * dataNegRespCode_u8)
{
    boolean dataDtcStatusflag_b = FALSE; /*Local variable to chk if event of Did in the loop was
                                                           either started/stopped/cleared*/
    Dcm_SesCtrlType dataCurrentSession_u8;
    Std_ReturnType dataReturnVal_u8 = E_NOT_OK; /* Local variable to store return from DcmAppl API */
...
static void Dcm_RoeLengthCheck(const Dcm_MsgContextType * pMsgContext,
        Dcm_NegativeResponseCodeType * dataNegRespCode_u8)
{
    uint16 dataReqLen_u16; /* Local variable to store Response Length */
...
{
#if(DCM_CFG_DSP_ROEDID_ENABLED!=DCM_CFG_OFF)
    uint8 dataidxLoop_u8;/*loop varaible*/
#endif
...
/**
 **************************************************************************************************
 * Function Name: Dcm_RoeCtrlConditionAndEventCheck:
 * Description  : This function is used to check if the control request leads to NRC 0x22,
 *                When a RoeEvent is in ROE started state and new request with subfunction
 *                onDTCStatusChange/onChangeOfDataIdentifier is requested.
 * Parameters   : dataNegRespCode_u8 (Parameter to update the NRC value)
 * Return Type  : void
 **************************************************************************************************
 */
static void Dcm_RoeCtrlConditionAndEventCheck(
        Dcm_NegativeResponseCodeType * dataNegRespCode_u8)
{
    Dcm_NegativeResponseCodeType ErrorCode_u8 = 0x00; /* To store the NRC */
#if(DCM_CFG_DSP_ROEONDTCSTATUSCHANGE_ENABLED != DCM_CFG_OFF)
    if(Dcm_stEventType_u8 == DCM_ROE_ONDTCSTATUSCHANGE)
    {
        /*Multiple client handling,atleast one of the event is started already and if the new request onDTCStatusChange
         *came from a different protocol then the request shall be rejected with NRC 0x22 */
        if((DCM_ROE_STARTED==DcmDspRoeDtcStateVariable.RoeEventStatus)&&
           (DcmDspRoeDtcStateVariable.SourceAddress_u16!=
                   Dcm_DsldConnTable_pcst[Dcm_DsldRxTable_pcu8[Dcm_DsldMsgContext_st.dcmRxPduId]].testaddr_u16))
        {
    ...
    if(Dcm_stEventType_u8 == DCM_ROE_ONCHANGEOFDID)
    {
        /*loop variable*/
        uint8 dataidxLoop_u8;
        /*Multiple client handling,atleast one of the event is started already and if the new request
         *onChangeOfDataIdentifier came from a different protocol then the request shall be rejected with NRC 0x22 */
        for(dataidxLoop_u8=0x0u; (dataidxLoop_u8<DCM_MAX_DIDROEEVENTS); dataidxLoop_u8++)
        {
            if((DCM_ROE_STARTED==DcmDspRoeDidStateVariables[dataidxLoop_u8].RoeEventStatus)&&
                    (DcmDspRoeDidStateVariables[dataidxLoop_u8].SourceAddress_u16!=
...
#if (DCM_CFG_RTESUPPORT_ENABLED != DCM_CFG_OFF)
            (*(DcmDspRoeDidEvents[Dcm_RoeDidIdx_u8].ROEDID_cpv.ROEDID_fp))(DCM_ROE_STOPPED);
#endif
...
#if (DCM_CFG_RTESUPPORT_ENABLED != DCM_CFG_OFF)
            (*(DcmDspRoeDidEvents[Dcm_RoeDidIdx_u8].ROEDID_cpv.ROEDID_fp))(DCM_ROE_STARTED);
#endif
...
static Std_ReturnType Dcm_Prv_DcmRoe_Controlevent (
                                                    Dcm_MsgContextType * pMsgContext,
                                   Dcm_NegativeResponseCodeType * dataNegRespCode_u8)
{
    Std_ReturnType dataReturnVal_u8; /* Local variable to store return from DcmAppl API */
...
static Std_ReturnType Dcm_Prv_DcmRoe_SetUpevent_WindowTime (
        Dcm_NegativeResponseCodeType * dataNegRespCode_u8)
{
    /*Multiple client handling,atleast one of the event is started already and if the new request onDTCStatusChange/
     *  onChangeOfDataIdentifier came from a different protocol then the request shall be rejected with NRC 0x22 */
...
static Std_ReturnType Dcm_Prv_DcmRoe_SetUpevent_ONCHANGEOFDID (
...
static Std_ReturnType Dcm_Prv_DcmRoe_SetUpevent_ONDTCSTATUSCHANGE (
                                     Dcm_MsgContextType * pMsgContext,
                                     Dcm_NegativeResponseCodeType * dataNegRespCode_u8)
{
    Std_ReturnType dataReturnVal_u8; /* Local variable to store return from DcmAppl API */
...
static Std_ReturnType Dcm_Prv_DcmRoe_SetUpevent (
        Dcm_MsgContextType * pMsgContext,
        Dcm_NegativeResponseCodeType * dataNegRespCode_u8)
{
    Std_ReturnType dataReturnVal_u8; /* Local variable to store return from DcmAppl API */
