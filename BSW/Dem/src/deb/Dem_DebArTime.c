...
/*  rework to reduce runtime			if (Dem_EvtGetDebounceLevel(EventId) > TimePassedThreshold) */
/* FC_VariationPoint_END */
		...
		case DEM_EVENT_STATUS_FAILED:
		{
			...
			TimeFailedThreshold = ((const Dem_DebArTime_ParamSet*) paramSet)[paramIndex].TimeFailedThreshold;
/* FC_VariationPoint_START */
/*  rework to reduce runtime			if (Dem_EvtGetDebounceLevel(EventId) < TimeFailedThreshold) */
/* FC_VariationPoint_END */
			{
				Dem_EvtSetDebounceLevel (EventId, (sint16) TimeFailedThreshold);
				if (!Dem_EvtSt_GetTestFailed(EventId))
	...
	if ((status2report == DEM_EVENT_STATUS_PASSED) || (status2report == DEM_EVENT_STATUS_FAILED))
	{
		...
		Dem_EvtProcessPassedAndFailed(EventId, status2report
                                      ...
                                      DEM_DEBUGDATA_PARAM(Dem_DebArTimeDebugValues[Dem_EvtParam_GetDebounceParamSettingIndex(EventId)][0],
                                                          Dem_DebArTimeDebugValues[Dem_EvtParam_GetDebounceParamSettingIndex(EventId)][1]));
#else
							          DEM_DEBUGDATA_PARAM(0 ,0));
#endif
	}
