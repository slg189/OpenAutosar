...
Std_ReturnType Dem_ReadDebug0(uint8* Buffer, const Dem_InternalEnvData* internalData, Dem_EncodingType DemEndianness)
{
    if(DemEndianness == DEM_CFG_LITTLE_ENDIAN)
    {
        Dem_LittleEndian_WriteValue2Buffer(Buffer, internalData->debug0, DEM_SIZEOF_TYPE(Dem_DebugDataType));
    }
    else
    {
...
Std_ReturnType Dem_ReadDebug1(uint8* Buffer, const Dem_InternalEnvData* internalData, Dem_EncodingType DemDemEndianness)
{
    if(DemDemEndianness == DEM_CFG_LITTLE_ENDIAN)
    {
        Dem_LittleEndian_WriteValue2Buffer(Buffer, internalData->debug1, DEM_SIZEOF_TYPE(Dem_DebugDataType));
    }
    else
    {
...
Std_ReturnType Dem_ReadEventStatusByte( uint8* buffer, const Dem_InternalEnvData* internalData, Dem_EncodingType DemEndianness )
{
    DEM_UNUSED_PARAM(DemEndianness);
...
Std_ReturnType Dem_ReadAgingCtrUpCnt(uint8* Buffer, const Dem_InternalEnvData* internalData, Dem_EncodingType DemEndianness)
{
    DEM_UNUSED_PARAM(DemEndianness);
...
Std_ReturnType Dem_ReadAgingCtrDownCnt(uint8* Buffer, const Dem_InternalEnvData* internalData, Dem_EncodingType DemEndianness)
{
    DEM_UNUSED_PARAM(DemEndianness);
    if (internalData->evMemLocation != NULL_PTR)
    {
...
Std_ReturnType Dem_ReadOccCtr(uint8* Buffer, const Dem_InternalEnvData* internalData, Dem_EncodingType DemEndianness)
{
    DEM_UNUSED_PARAM(DemEndianness);
	if (internalData->evMemLocation != NULL_PTR)
	{
...
Std_ReturnType Dem_ReadOvflInd(uint8* Buffer, const Dem_InternalEnvData* internalData, Dem_EncodingType DemEndianness)
{
    DEM_UNUSED_PARAM(DemEndianness);
...
Std_ReturnType Dem_ReadSignificance(uint8* Buffer, const Dem_InternalEnvData* internalData, Dem_EncodingType DemEndianness)
{
    DEM_UNUSED_PARAM(DemEndianness);
...
Std_ReturnType Dem_ReadCurrentFDC(uint8* Buffer, const Dem_InternalEnvData* internalData, Dem_EncodingType DemEndianness)
{
    DEM_UNUSED_PARAM(DemEndianness);
...
Std_ReturnType Dem_ReadMaxFdcDuringCurrentCycle(uint8* Buffer, const Dem_InternalEnvData* internalData, Dem_EncodingType DemEndianness)
{
    DEM_UNUSED_PARAM(DemEndianness);
	if (internalData->evMemLocation != NULL_PTR)
	{
...
Std_ReturnType Dem_ReadMaxFdcSinceLastClear(uint8* Buffer, const Dem_InternalEnvData* internalData, Dem_EncodingType DemEndianness)
{
    DEM_UNUSED_PARAM(DemEndianness);
	if (internalData->evMemLocation != NULL_PTR)
	{
...
Std_ReturnType Dem_ReadCyclesSinceFirstFailed(uint8* Buffer, const Dem_InternalEnvData* internalData, Dem_EncodingType DemEndianness)
{
    DEM_UNUSED_PARAM(DemEndianness);
	if (internalData->evMemLocation != NULL_PTR)
	{
...
Std_ReturnType Dem_ReadCyclesSinceLastFailed(uint8* Buffer, const Dem_InternalEnvData* internalData, Dem_EncodingType DemEndianness)
{
    DEM_UNUSED_PARAM(DemEndianness);
	if (internalData->evMemLocation != NULL_PTR)
	{
...
Std_ReturnType Dem_ReadCyclesSinceLastFailedExcludingTNC(uint8* Buffer, const Dem_InternalEnvData* internalData, Dem_EncodingType DemEndianness)
{
    DEM_UNUSED_PARAM(DemEndianness);
	if (internalData->evMemLocation != NULL_PTR)
	{
...
Std_ReturnType Dem_ReadFailedCycles(uint8* Buffer, const Dem_InternalEnvData* internalData, Dem_EncodingType DemEndianness)
{
    DEM_UNUSED_PARAM(DemEndianness);
	if (internalData->evMemLocation != NULL_PTR)
	{
...
Std_ReturnType Dem_ReadEventId(uint8* Buffer, const Dem_InternalEnvData* internalData, Dem_EncodingType DemEndianness)
{
    if(DemEndianness == DEM_CFG_LITTLE_ENDIAN)
        {
            Dem_LittleEndian_WriteValue2Buffer(Buffer, internalData->eventId, DEM_SIZEOF_TYPE(Dem_EventIdType));
        }
        else
        {
