...
void Nm_CoordReadyToSleepIndication(NetworkHandleType NetworkHandle)
{
    ...
    if (Nm_NetworkHandle_u8 < NM_NUMBER_OF_CHANNELS)
    {
        ...
        if((ConfDataPtr_pcst->ClusterIdx != 0xFF))
        {
            ...
            if((ConfDataPtr_pcst->NmChannelType == NM_PASSIVE_CHANNEL) && (ConfDataPtr_pcst->NmChannelSleepMaster == FALSE))
            {
                ...
                if((MainNetworkRamDataPtr_pst->ChannelState_en == NM_CHANNEL_STATE_WAIT_SLEEP) && (MainNetworkRamDataPtr_pst->RemoteSleepStatus_b == FALSE))
                {
    ...
    else
    {
        ...
        NM_DET_REPORT_ERROR(NetworkHandle, NM_SID_COORDREADYTOSLEEPINDICATION, NM_E_INVALID_CHANNEL);
#else
