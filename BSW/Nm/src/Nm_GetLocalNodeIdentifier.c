...
Std_ReturnType Nm_GetLocalNodeIdentifier(NetworkHandleType NetworkHandle,uint8 * nmNodeIdPtr)
{
    ...
    if ((Nm_NetworkHandle_u8 < NM_NUMBER_OF_CHANNELS) && (nmNodeIdPtr != NULL_PTR))
    {
        ...
        /* Holds the reference to Bus specific APIs */
	        ...
	        RetVal_u8 = (*FuncPtr_pcst->BusNm_GetLocalNodeIdentifier_pfct)(NetworkHandle, nmNodeIdPtr);
    ...
    else
    {
        ...
        if (!(Nm_NetworkHandle_u8 < NM_NUMBER_OF_CHANNELS))
        {
            /* Report to DET as the network handle parameter is not a configured network handle */
             NM_DET_REPORT_ERROR(NetworkHandle, NM_SID_GETLOCALNODEIDENTIFIER, NM_E_INVALID_CHANNEL);
        }
        else
        {
           //do nothing
