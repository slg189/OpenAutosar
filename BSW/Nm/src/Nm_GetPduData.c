...
Std_ReturnType Nm_GetPduData(NetworkHandleType NetworkHandle, uint8 * nmPduData)
{
    ...
    if ((Nm_NetworkHandle_u8 < NM_NUMBER_OF_CHANNELS) && (nmPduData != NULL_PTR))
    {
        ...
        /* Holds the reference to Bus specific APIs */
	        ...
	        RetVal_u8 = (*FuncPtr_pcst->BusNm_GetPduData_pfct)(NetworkHandle, nmPduData);
    ...
    else
    {
        if (!(Nm_NetworkHandle_u8 < NM_NUMBER_OF_CHANNELS))
        {
            /* Report to DET as the network handle parameter is not a configured network handle */
             NM_DET_REPORT_ERROR(NetworkHandle, NM_SID_GETPDUDATA, NM_E_INVALID_CHANNEL);
        }
        else
        {
            //do nothing
...
Std_ReturnType Nm_GetPduData(NetworkHandleType NetworkHandle, uint8 * nmPduData)
{
    ...
    if ((Nm_NetworkHandle_u8 < NM_NUMBER_OF_CHANNELS) && (nmPduData != NULL_PTR))
    {
        ...
        /* Holds the reference to Bus specific APIs */
		if((NM_USER_DATA_ENABLED != STD_OFF) || ((ConfDataPtr_pcst->NmNodeIdEnabled) == TRUE))
		{
			FuncPtr_pcst = &Nm_BusNmApi[ConfDataPtr_pcst->BusNmType];
