...
#define XCP_SRV_BUFFER_PTR(a)     Xcp_Cleared[a].ServBuffer.Buffer_au8
#define XCP_SRV_BUFFER_LENGTH(a)  Xcp_Cleared[a].ServBuffer.Length_u32
...
#define XCP_PLCFG_EV(EventChannelNum)                       Xcp_PlCfgConst.EventChannelCfg[EventChannelNum]
#define XCP_PLCFG_EV_SIZE(EventChannelNum)                  XCP_PLCFG_EV(EventChannelNum).ODTDAQBufferElementSize_u16
#define XCP_PLCFG_EV_RESERVE(EventChannelNum)               XCP_PLCFG_EV(EventChannelNum).ODTDAQBufferElementReserve_u8

#if (XCP_BYPASS_CONSISTENCY == STD_ON)
#define XCP_BYPASS(EventChannelNum)                         Xcp_EventChannel_BypassConsistency.Xcp_BypassConsistency[EventChannelNum]
#endif

#define Xcp_Daq_QueueCalcParam(EventChannelNum)             (uint32)((XCP_PLCFG_EV_SIZE(EventChannelNum)*100) + ((XCP_PLCFG_EV_RESERVE(EventChannelNum)* XCP_PLCFG_EV_SIZE(EventChannelNum))))
#define Xcp_Daq_OdtElementsInDaqQue(OdtCnt, EventChannelNum)         (((OdtCnt * Xcp_Daq_QueueCalcParam(EventChannelNum))/100u) +  ((((OdtCnt * Xcp_Daq_QueueCalcParam(EventChannelNum)) % 100u) + 50u) / 100u) + 1u)
#else
#define Xcp_Daq_OdtElementsInDaqQue(OdtCnt)                 (uint32)((OdtCnt * XCP_ODT_DAQ_BUFFER_ELEMENT_SIZE) + 1u)
#endif

#if (XCP_VERSION_ABOVE_1_3 == STD_ON)
#define XCP_PLCFG_EV_PACKEDFLAG(EventChannelNum)            XCP_PLCFG_EV(EventChannelNum).DaqListPropertiesPackedFlag_u8
#endif
/** \} */
...
typedef enum
{
  XCP_BG_IDLE                       = 0x00u,                /**< No background activity running  */
  XCP_BG_CHKSUM                     = 0x01u,                /**< Background activity BUILD_CHECKSUM is running  */
  XCP_BG_MEM_WRITE                  = 0x02u,                /**< Background activity DOWNLOAD is running  */
  XCP_BG_REPEAT_CMD                 = 0x03u,                /**< Last command shall be repeated as it was not finished yet (pending) */
  XCP_BG_DO_DISCONNECT              = 0x04u,                /**< Stop all DaqLists and then disconnect the PL */
  XCP_BG_CANCEL_REQ                 = 0x05u                 /**< Running background activity is cancelled  */
} Xcp_BgActivity_t;
...
typedef struct
{
# if (XCP_DAQ_PACKED_MODE_SUPPORTED == STD_ON)              /**< destination pointer for copy ODT in packed mode */
  uint8*          Dest_pu8;                                 /**< */
#endif
...
# if (XCP_DAQ_PACKED_MODE_SUPPORTED == STD_ON)
  uint16 DpmSampleCount_u16;                                    /**< This parameter is the number of samples, which are packed into one DAQ list transmission */
  uint16 DpmCurrentSampledTime_u16;                             /**< This parameter defines the current sampled time, it increases after each sampling */
  uint8  DaqPackedMode_u8;                                     /**< 0 = data not packed; 1 = element-grouped data packing; 2 = event-grouped data packing; rest not supported*/
  uint8  DpmTimestampMode_u8;                                  /**< This parameter defines the number and meaning of timestamps in packed mode */
# endif
}Xcp_DaqList_t;
...
typedef enum
{
  XCP_NVM_NO_JOB                        = 0x00u,            /**< No Write or Clear is processed */
  XCP_NVM_JOB_RUNNING                   = 0x01u             /**< NvM Write or Clear request is processed */
}Xcp_NvMState_t;

/** Resume mode request status  *****************************************************************/
typedef enum
{
  XCP_STORE_IDLE                        = 0x00u,               /**< No SET_REQUEST was received */
  XCP_STORE_MODE_WRITE                  = 0x01u,               /**< XCP_STORE_DAQ_REQ was requested and TLData block is cleared */
  XCP_STORE_MODE_BLOCK_INFO_WRITE       = 0x02u,               /**< XCP_STORE_DAQ_REQ was requested and TLData block is written */
  XCP_STORE_MODE_BLOCK_DATA_WRITE       = 0x03u,               /**< XCP_STORE_DAQ_REQ was requested and DAQ config block is written */
  XCP_STORE_MODE_CLEAR                  = 0x04u,               /**< XCP_CLEAR_DAQ_REQ was requested and TLData block is cleared */
  XCP_STORE_MODE_BLOCK_DATA_CLEAR       = 0x05u                /**< XCP_CLEAR_DAQ_REQ was requested and DAQ config block is cleared */
...
typedef struct
{
  ...
  Xcp_CtoMax_t      ServBuffer;                              /**< Service data buffer of size MAX_CTO with response length */
#if (XCP_BLOCK_TRANSFER_UPLOAD == STD_ON || XCP_BLOCK_TRANSFER_DOWNLOAD == STD_ON || XCP_UPLOAD_DOWNLOAD_MEMORY_ACCESS_BY_APPL == STD_ON)
  ...
  uint16            ReqTimeoutCnt_u16;                          /**< To detect the checksum build timeout */
#endif
#if (XCP_DEV_ERROR_DETECT == STD_ON)
...
extern void   Xcp_TxConfirmation(uint8 XcpTransportLayerId, Std_ReturnType result);
#else
...
#if (XCP_BYPASS_CONSISTENCY == STD_ON)
extern void Xcp_ConsistencyCheck(uint16 EventChannelNum, uint8 protLayerId);
#endif
#endif /* end of XCP_SYNCHRONOUS_DATA_STIMULATION_STIM */
#if (XCP_SYNCHRONOUS_DATA_ACQUISITION_DAQ == STD_ON)
...
/**
****************************************************************************************************
Function processes the SET_DAQ_PACKED_MODE sub-command
\param[in]  protLayerId   Protocol Layer Id
\return     -
***************************************************************************************************/
extern void Xcp_CmdSetDaqPackedMode            (const PduInfoType* XcpPacket, uint8 protLayerId );
...
/**
****************************************************************************************************
Function processes the GET_DAQ_PACKED_MODE sub-command
\param[in]  protLayerId   Protocol Layer Id
\return     -
***************************************************************************************************/
extern void Xcp_CmdGetDaqPackedMode            (const PduInfoType* XcpPacket, uint8 protLayerId );

extern uint8 Xcp_DaqEventSampleNext(uint16 daqListNo_u16, uint8 protLayerId);

#endif /* XCP_DAQ_PACKED_MODE_SUPPORTED == STD_ON */
