...
#define XCP_BLOCK_PDU(PduId)      do { Xcp_FrCfgCleared.TxPduBlocked_ab[PduId] = TRUE; Xcp_FrCfgCleared.TxPduMonitorCnt[PduId] = 1u;} while (0)
/** Clear the given Pdu and mark it as free */
#define XCP_EMPTY_PDU(PduId)      do { Xcp_FrCfgCleared.TxPdu[PduId].SduLength = 1; Xcp_FrCfgCleared.TxPduBlocked_ab[PduId] = FALSE; Xcp_FrCfgCleared.TxPduMonitorCnt[PduId] = 0u;} while (0) /* 1 because on position 0 is always NAX */
...
void Xcp_FrIfTxConfirmation(PduIdType XcpTxPduId, Std_ReturnType result)
#else
  ...
  if (Xcp_FrCfgNoInit.ConnectedTLCtr_u8 > 0)
  {
    ...
    for (i = 0 ; i < XCP_FR_NUMBER_OF_TRANSPORT_LAYERS ; i++)
    {
            ...
            )
      {
        ...
        /* MR12 RULE 2.2 VIOLATION: XCP_*_TRANSPORT_LAYER_INDEX_OFFSET changes depending on the configuration thus the calculation is necessary if the value is not 0 (0 raises the MISRA violation) */
#if((XCP_AR_RELEASE_MAJOR_VERSION == 4u) && (XCP_AR_RELEASE_MINOR_VERSION > 2u))
        Xcp_TxConfirmation(i + XCP_FR_TRANSPORT_LAYER_INDEX_OFFSET, E_OK);
#else
    ...
    /* run the TxPdu timeout monitor */
    for (PduId = 0 ; PduId < XCP_FR_NUMBER_OF_TX_PDUS ; PduId++)
    {
      if(Xcp_FrCfgCleared.TxPduMonitorCnt[PduId] != 0u)
      {
        Xcp_FrCfgCleared.TxPduMonitorCnt[PduId]++;
        /* Empty Pdu when timeout */
        if(Xcp_FrCfgCleared.TxPduMonitorCnt[PduId] == XCP_FR_TXPDU_TXCONF_CNT)
        {
          XCP_EMPTY_PDU(PduId);
        }
      }
...
Std_ReturnType Xcp_FrIfTriggerTransmit(PduIdType TxPduId, PduInfoType* PduInfoPtr)
{
  ...
  /* If XcpRxPduPtr is NULL_PTR, report to DET and exit the function */
  XCP_DET_REPORT_ERROR_RET((PduInfoPtr == NULL_PTR), XCP_TRIGGER_TRANSMIT_SID, XCP_E_PARAM_POINTER, E_NOT_OK);
  /* If Invalid TxPduId, report to DET and exit the function */
