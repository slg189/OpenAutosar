...
void Xcp_CmdGetSeed(const PduInfoType* XcpPacket, uint8 protLayerId)
{
  ...
  else if(CmdPtr->Mode_u8 == 0x01)
  {
    ...
    if (Xcp_Cleared[protLayerId].SeedAndKey.SeedRemaingSize_u8 > 0u)
    {
      ...
      /* MR12 RULE 11.4 VIOLATION: Cast from XCP_ADDR_TYPE to pointer is OK as XCP_ADDR_TYPE is able to store the address (see type definition) */
  ...
  if (Error == XCP_NO_ERROR)
  {
    ...
    /* MR12 RULE 11.4 VIOLATION: Cast from pointer to XCP_ADDR_TYPE is OK as XCP_ADDR_TYPE is able to store the address (see type definition) */
