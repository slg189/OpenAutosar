...
void Xcp_CmdSetDaqListMode(const PduInfoType* XcpPacket, uint8 protLayerId)
{
  ...
  else if ((CmdPtr->Mode_u8 & ((uint8) (~(XCP_DAQLISTMODE_DIRECTION|XCP_DAQLISTMODE_TIMESTAMP|XCP_DAQLISTMODE_DTO_CTR)))) != 0u)
  {
#else
...
#  if (XCP_DAQ_PACKED_MODE_SUPPORTED == STD_ON)
    else if(XCP_DAQ(CmdPtr->DaqListNum_u16, protLayerId).DaqPackedMode_u8 != 0u)
    {
...
#  endif  /* XCP_SEED_AND_KEY == STD_ON */
    ...
    {
        /* nothing to do. Queue memory calculation performed later */
    }
# endif    /* XCP_SYNCHRONOUS_DATA_STIMULATION_STIM */
  }

#if (XCP_DAQ_PACKED_MODE_SUPPORTED == STD_ON)
  /* Timestamp must be enabled if daq packed mode is configured and also Packed flag bit should be set to 1 */
  else if((((CmdPtr->Mode_u8 & XCP_DAQLISTMODE_TIMESTAMP) == 0u) && (XCP_DAQ(CmdPtr->DaqListNum_u16, protLayerId).DaqPackedMode_u8 != 0u)) || (XCP_PLCFG_EV_PACKEDFLAG(CmdPtr->EventChannelNum_u16) == 0u) )
  {
    Error = XCP_ERR_DAQ_CONFIG;
  }
#endif
  ...
  if (Error == XCP_NO_ERROR)
  {
    ...
    /* Check if sufficient memory is available for the DAQ list, after queue calculation */
