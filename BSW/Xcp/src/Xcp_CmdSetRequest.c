...
static void Xcp_EvStoreCal(uint8 protLayerId);
...
void Xcp_CmdSetRequest(const PduInfoType* XcpPacket, uint8 protLayerId)
{
  ...
  {
    if (CmdPtr->Mode_u8 == XCP_SET_REQUEST_STORE_CAL_REQ)
    {
#if (XCP_STORE_CAL_SUPPORTED == STD_ON)
      /* Set the session information to XCP_STORE_DAQ_REQ */
      XCP_RESUME_SESSION_STATUS(protLayerId) = XCP_STORE_CAL_REQ;
      /* call function to store calibration request */
      Error = XcpAppl_StoreCalReq();

      if (Error == XCP_NO_ERROR)
      {
        /* call function to reset session bit and send Event packet for STORE_CAL_REQ */
        Xcp_EvStoreCal(protLayerId);
      }
#else
      /* STORE_CAL_REQ not supported right now. return error */
      Error = XCP_ERR_OUT_OF_RANGE;
#endif
    }
#if (XCP_STORE_CLEAR_DAQ_SUPPORTED == STD_ON)
    else if (CmdPtr->Mode_u8 == XCP_SET_REQUEST_STORE_DAQ_REQ_RESUME)
    {
      /* Set the session information to XCP_STORE_DAQ_REQ */
...
void Xcp_ResumeModeActivate(void)
{
  ...
  boolean newSwFlashed;
  /* Check if a new software is flashed before the XCP initialization*/
  newSwFlashed =  XcpAppl_NewSwFlashed();
#endif
  ...
  ErrorState = Xcp_NvMReadBlock(XCP_BLOCK_INFO_INDEX, (void*)&Xcp_GlobalNoInit.SetRequestData.TLData_t);
     ...
     )
  {
    ...
    if (transLayerId != 0xFF)
    {
      ...
      else
      {
        ...
        XCP_DET_REPORT_ERROR_NO_RET(XCP_INITIALIZE_SID, XCP_E_PARAM_POINTER);
      }
    }
    else
    {
      /* Read ended with error - call DET to signal that no valid data was restored */
      XCP_DET_REPORT_ERROR_NO_RET(XCP_INITIALIZE_SID, XCP_E_INVALID_TRANSPORT_LAYER_ID);
    }
  }
  else
  {
    /* Read ended with error */
    /* This can happen after a previous CLEAR_DAQ_REQ as well since this is invalidating the NvM block for a next read */

    /* Invalidate the NvM INFO block when the resume mode is activated and a new software is flashed
     * This can avoid XCP entering the resume mode unintentionally in the next XCP initialization */
#if (XCP_RESUME_MODE_FLASH_CHECK == STD_ON)
    if(ErrorState == E_OK)
    {
      /* Request the erase of TransportLayer information and DAQ size block from NvM */
      ErrorState = NvM_InvalidateNvBlock(Xcp_PlCfgConst.NvMBlockInfo[XCP_BLOCK_INFO_INDEX].BlockId);
      /* When invalidate Nv Block failed */
      if (ErrorState == E_NOT_OK)
      {
        XCP_DET_REPORT_ERROR_NO_RET(XCP_INITIALIZE_SID, XCP_E_INVALIDATE_NVBLOCK_FAILED);
      }
    }
#endif
  }
    ...
    case XCP_NVM_NO_JOB:
    {
      ...
      switch (Xcp_GlobalNoInit.SetRequestData.StorageMode)
      {
        ...
        case XCP_STORE_MODE_BLOCK_DATA_CLEAR:
        {
          ...
          ErrorState = NvM_InvalidateNvBlock(Xcp_PlCfgConst.NvMBlockInfo[XCP_BLOCK_DATA_INDEX].BlockId);
...
static Xcp_ErrorCode Xcp_PrepareNVMBlock(uint8 protLayerId)
{
  ...
  else
  {
    ...
    MaxRamCopySize = XCP_MIN(Xcp_NoInit[protLayerId].DaqConfig.DaqRamSize_u32, Xcp_PlCfgConst.NvMBlockInfo[XCP_BLOCK_DATA_INDEX].BlockLength_u16);

    /* Check if enough space to copy in the NvM INFO block */
    if (sizeof(Xcp_SmallBlockData_t) <= Xcp_PlCfgConst.NvMBlockInfo[XCP_BLOCK_INFO_INDEX].BlockLength_u16)
    {
      ...
      if (Error == XCP_NO_ERROR)
      {
        ...
        if ((BlockSize <= MaxRamCopySize))
        {
          /* MR12 RULE 11.5, DIR 1.1 VIOLATION: The pointer cast is required for copying the data from the Daq pointer to local structure */
...
static void Xcp_RestoreDaqConfig(uint8 protLayerId)
{
  ...
  /* MR12 RULE 11.5, DIR 1.1 VIOLATION: The pointer cast is required for copying the data from local structure to Daq pointer */
...
#endif /* #if (XCP_STORE_CLEAR_DAQ_SUPPORTED == STD_ON) */

#if (XCP_STORE_CAL_SUPPORTED == STD_ON)
/**
****************************************************************************************************
This service sends the event EV_STORE_CAL and resets the Session bit when calibration data has been
successfully saved in memory.
\param[in]  protLayerId   Protocol Layer Id
\return     -
***************************************************************************************************/
static void Xcp_EvStoreCal(uint8 protLayerId)
{
  /* Clear the STORE_CAL_REQ bit that is received by the GET_STATUS */
  XCP_RESUME_SESSION_STATUS(protLayerId) = XCP_RESUME_SESSION_STATUS(protLayerId) & (uint8)(~XCP_STORE_CAL_REQ);
  /* send out XCP_EV_STORE_CAL event */
  Xcp_SendEv_Code(XCP_EV_STORE_CAL, protLayerId);
}
#endif
