...
Xcp_ErrorCode XcpAppl_BuildChecksumMainFunction(uint32* ChecksumPtr, uint8* ChecksumType, uint8 ProtocolLayerId)
{
  ...
  if (XcpAppl_BuildChecksumData[ProtocolLayerId].BlockSize_u32 > 0u)
  {
    ...
    if (XcpAppl_BuildChecksumData[ProtocolLayerId].XcpAddress != 0u)
    {
      ...
      XcpAppl_BuildChecksumData[ProtocolLayerId].XcpAddress = 0u;
    }
...
Xcp_ErrorCode XcpAppl_MemWrite(Xcp_AddrType_t XcpAddrDst, const uint8* AddrPtrSrc, uint8 Length, uint8 ProtocolLayerId)
{
  ...
  /* MR12 RULE 11.4 VIOLATION: rba_Dme_Write wants as first param an uint32 (address) but not a pointer, so we have to cast, which is not a problem as addresses are also uint32;
                               the third argument needs to be a uint8-pointer, AddrPtrSrc is const - to avoid compiler-warning we cast to XCP_ADDR_TYPE first */
  DmeStatus = rba_Dme_Write(XcpAddrDst.AddrValue, XcpAppl_Data[ProtocolLayerId].CalPage_u8, Length, (uint8*)((XCP_ADDR_TYPE)AddrPtrSrc), XcpAppl_DmeUser[ProtocolLayerId]);
