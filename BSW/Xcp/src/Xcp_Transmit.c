...
static void Xcp_SendSrvTxConf(uint8 protLayerId);
static void Xcp_SendServiceRequest(uint8 protLayerId);
...
static Std_ReturnType Xcp_Transmit(const PduInfoType* XcpPacketPtr, uint8 protLayerId, Xcp_PduIdType XcpTxPduId)
{
  ...
  if (Status == E_OK)
  {
    ...
    XCP_DEBUG_CTR_INC(Xcp_Cleared[protLayerId].Debug.Xcp_Debug_TransmitOkCtr_u16);
  }
  else
  {
    /* Transmit not successful, do not expect a tx_confirmation.
       Wait4TxConfCtr is set to 0 by caller of this function (required). */
    XCP_DEBUG_CTR_INC(Xcp_Cleared[protLayerId].Debug.Xcp_Debug_TransmitNotOkCtr_u16);
  }
...
static void Xcp_SendResTxConf(uint8 protLayerId)
{
  ...
  XCP_DEBUG_CTR_INC(Xcp_Cleared[protLayerId].Debug.Xcp_Debug_SendResTxConfCtr_u16);
...
/**
****************************************************************************************************
Function sends a service packet to the Master in order to transfer information in data packets
\param[in]  protLayerId       protocol layer id
\return     -
***************************************************************************************************/
static void Xcp_SendServiceRequest(uint8 protLayerId)
{
    ...
    if (Xcp_Cleared[protLayerId].Wait4TxConfCtr_u8 == 0)
    {
      ...
      /* Prepare XcpPacket for transmission */
      XcpPacket.SduDataPtr = XCP_SRV_BUFFER_PTR(protLayerId);
      XcpPacket.SduLength  = (PduLengthType) XCP_SRV_BUFFER_LENGTH(protLayerId);
      ...
      else
      {
        ...
        if (XCP_SRV_BUFFER_LENGTH(protLayerId) == 0u)
        {
          /* reset the buffer length as transmit failed */
          XCP_SRV_BUFFER_LENGTH(protLayerId) = XcpPacket.SduLength;
        }
        /* else: other service packet is in buffer, do nothing */
...
void Xcp_SendRes(uint8 protLayerId)
{
  ...
  SchM_Enter_Xcp_SendingShort();
  XCP_DEBUG_CTR_INC(Xcp_Cleared[protLayerId].Debug.Xcp_Debug_SendResCtr_u16);
...
/**
****************************************************************************************************
Function sends a service packet to the Master in order to reset the ECU
\param[in]  protLayerId       protocol layer id
\return     -
***************************************************************************************************/
void Xcp_SendServiceRequestReset(uint8 protLayerId)
{
  ...
  Xcp_ResService_t* SrvPtr = (Xcp_ResService_t*) (void*) XCP_SRV_BUFFER_PTR(protLayerId);

  /* Fill the service buffer with the packet ID and service code */
  XCP_SRV_BUFFER_LENGTH(protLayerId) = 2u;
  SrvPtr->PacketId_u8 = 0xFCu;
  SrvPtr->ServiceReqCode_u8 = 0x00;


  /* Send the service request data */
  Xcp_SendServiceRequest(protLayerId);
...
/**
****************************************************************************************************
Function sends a service packet containing ASCII text to the Master in order to transfer information in data packets
\param[in]  protLayerId       protocol layer id
\return     -
***************************************************************************************************/
/* MR12 RULE 8.13 VIOLATION: The pointer SrvPtr is copied from DataStream and it's contents are modified - therefore it cannot be const. */
void Xcp_SendServiceRequestData(uint8 *DataStream, uint8 StreamSize_u8, uint8 protLayerId)
{
    /* Local variables */
    uint8 i = 0;
    uint16 ServiceDataSize;
    ...
    /* Store the size of the data section of the CTO packet */
    ServiceDataSize = XCP_MAXCTO(protLayerId) - 2u;

    /* Fill the packet ID and service code bytes in the buffer */
    SrvPtr->PacketId_u8 = 0xFCu;
    SrvPtr->ServiceReqCode_u8 = 0x01;

    /* Check if the stream data can fit in one XCP packet */
    if( StreamSize_u8 <= ServiceDataSize)
    {
        /* Initialize the size of the service buffer. Make it equal to stream size + 2 of the data stream */
        XCP_SRV_BUFFER_LENGTH(protLayerId) = StreamSize_u8 + 2u;
    }
    else
    {
        /* Initialize the size of the service buffer. Make it equal to MAX CTO of connected TL */
        XCP_SRV_BUFFER_LENGTH(protLayerId) = XCP_MAXCTO(protLayerId);
    }

    /* Copy the data from the data stream into the Service buffer */
    for (i = 0; i < (XCP_SRV_BUFFER_LENGTH(protLayerId) - 2u); i++)
    {
        SrvPtr->OptionalServData_au8[i] = DataStream[i];
    }
...
static void Xcp_SendEvTxConf(uint8 protLayerId)
{
  ...
  /* Prepare XcpPacket for transmission */
  SchM_Enter_Xcp_SendingShort();
  XCP_DEBUG_CTR_INC(Xcp_Cleared[protLayerId].Debug.Xcp_Debug_SendEvTxConfCtr_u16);
...
/**
****************************************************************************************************
This function sends a service out of a tx confirmation. \n
It runs in TxConfirmation --> interrupt context. It cannot be interrupted by another TxConfirmation
\param[in]  protLayerId   protocol layer id
\return     -
***************************************************************************************************/
...
void Xcp_SendEv(const Xcp_Cto8_t* XcpPacketCtoPtr, uint8 protLayerId)
{
  ...
  SchM_Enter_Xcp_SendingShort();
  XCP_DEBUG_CTR_INC(Xcp_Cleared[protLayerId].Debug.Xcp_Debug_SendEvCtr_u16);
...
static void Xcp_SendDaqTxConf(Xcp_DaqList_t* DaqListPtr, uint8 protLayerId)
{
  ...
  uint16      ReadPos;
  PduInfoType XcpPacket;
  XCP_DEBUG_CTR_INC(Xcp_Cleared[protLayerId].Debug.Xcp_Debug_SendDaqTxConfCtr_u16);
...
void Xcp_SendDaq(uint16 daqListNo_u16, uint8 protLayerId)
{
  ...
  SchM_Enter_Xcp_SendingShort();
  XCP_DEBUG_CTR_INC(Xcp_Cleared[protLayerId].Debug.Xcp_Debug_SendDaqCtr_u16);
...
static Std_ReturnType Xcp_DaqQueFindNextOdtToSend(Xcp_DaqList_t** DaqListPtr, uint8 protLayerId)
{
  ...
  uint16_least i;
  /* MR12 RULE 8.13 VIOLATION: The referenced object is modifiable and is passed to a non-const object, thus const is not used */
...
void Xcp_TxConfirmation(uint8 XcpTransportLayerId, Std_ReturnType result)
#else
...
#endif
{
  ...
  uint8 protLayerId;
#if (XCP_SYNCHRONOUS_DATA_ACQUISITION_DAQ == STD_ON)
  Xcp_DaqList_t* DaqListPtr = NULL_PTR;
#endif
  ...
  if (protLayerId != XCP_INVALID_PROTLAYER_ID)
  {
    ...
    SchM_Enter_Xcp_SendingShort();
    XCP_DEBUG_CTR_INC(Xcp_Cleared[protLayerId].Debug.Xcp_Debug_TxConfCtr_u16);
    ...
    /* SERV */
    else if (XCP_SRV_BUFFER_LENGTH(protLayerId) > 0u)
    {
      SchM_Exit_Xcp_SendingShort();
      Xcp_SendSrvTxConf(protLayerId);
    }
