...
static void __Core0_start(void)
{
    ...
    // Ifx_Ssw_MTCR(CPU_BTV, (unsigned int)__TRAPTAB(0));  //Will be initialized by OS
    /* Base interrupt vector table initialized */
    // Ifx_Ssw_MTCR(CPU_BIV, (unsigned int)__INTTAB(0));   //Will be initialized by OS
...
{
    unsigned int coreID;
    coreID = GetCoreID();//__MFCR(CORE_ID_ADDR); // get the core ID from CPU_ID
    if(coreID == 0u)
    {
