...
`cflags`/`defines`/`includes`，以及 `extra_objects`、`generated_source_dirs`、`generated_include_dirs` 等 Gen/交付对象输入；命令行可临时覆盖工具链/平台/构建类型：
...
> 模块级声明在 `build.yaml`（`enabled` / `mode` / `cflags` / `extra_objects` / `generated_source_dirs` …）；`only/skip/layers` 为命令行临时过滤。

每个模块可配 `mode`（`source` 源码编译 / `lib` 库集成）、`enabled`、`cflags`（模块级编译选项）。供应商只交付 `.o` 时用 `extra_objects`；配置工具在 `Projects/<P>/Gen/<Layer>/<Module>/src` 生成代码时用 `generated_source_dirs`，对应头目录用 `generated_include_dirs` 或 `public_includes`。

框架对每个模块给出三种结果（`mode` 留空时自动判定，可显式覆盖）：

| 结果 | 触发条件 | 行为 | release |
|------|----------|------|---------|
| `source` | 有 `src/*.c`（或 `.S`），或配置了 `extra_sources` / `extra_source_dirs` / `extra_objects` / `generated_source_dirs` | 编译/归档：模块静态代码 + Gen 配置代码 + 交付 `.o` → `Libs/<Layer>/lib<Mod>.a` | `.a` + `inc/*.h` + `release_includes` 列出的文件 → `<Layer>_Libs/<Mod>/` |
...
> - `CDD/<Mod>/obj/*.o`（供应商只交付对象文件、无 `.c`）→ 在 `build.yaml` 写 `extra_objects`
> - `Projects/<P>/Gen/<Layer>/<Mod>/src/*.c`（配置工具生成的模块配置代码）→ 在 `build.yaml` 写 `generated_source_dirs`，会与模块静态代码一起归档为一个 `.a`
> - 配置/生成头（如 `CDD/CddPwm/gen/CddPwm_Cfg.h`、`Gen/BSW/CanIf/CanIf_Cfg.h`）**不会自动收集**，
>   需在 `build.yaml` 的 `generated_include_dirs` / `public_includes` 中加入编译可见路径，发布时再用 `release_includes` 显式列出具体文件路径
...
| `Projects/<P>/misra.json` | 项目级 MISRA C:2012 静态检查配置（根目录 `misra.json` 仅作兼容回退） |
...
scons gen:etas_bsw PROJECT=Projects/Demo_Tc387 REM 启用 etas_bsw 后调用 ISOLAR-B 生成 BSW
...
```bat
Projects\Demo_Tc387\08_gen_bsw_etas.bat --dry-run
set ISOLAR_B_CMD=C:\ETAS\ISOLAR-B\ISOLAR-B.cmd
Projects\Demo_Tc387\08_gen_bsw_etas.bat
```

对应配置在 `Projects/Demo_Tc387/build.yaml` 的 `codegen.generators.name=etas_bsw`，默认把 `Projects/Demo_Tc387/Cfg/BSW` 作为 `-data` / `-p` 配置路径并输出到 `Projects/Demo_Tc387/Gen/BSW`；如当前 ISOLAR-B 版本要求 `-p` 填工程名，可把 `project` 改成工程名。
